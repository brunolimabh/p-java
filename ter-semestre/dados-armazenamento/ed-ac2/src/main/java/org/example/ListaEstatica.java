...
//            }
    ...
    public void adicionaNoMeio(int elementoNovo) {
        if (nroElem >= vetor.length || nroElem == 0) throw new IllegalStateException();
        int meio = nroElem / 2;
        for (int i = vetor.length - 1; i >= meio; i--) {
            if (i == meio){
    ...
    /* Método somaImparRecursivo - percorre a lista de forma recursiva, retornando a soma dos valores
     *  impares que estao na lista. Fiquem a vontade para criar outro metodo private, caso achem necessario
     *
     * */
    public int somaImparRecursivo() {
        return somaImparRecursivo(0,0);
    }

    // TODO: estou acostumado a retornar um "sout", tentei fazer do mesmo modo mas não deu certo
    private int somaImparRecursivo(int indice, int soma){
        if (indice >= vetor.length){
            System.out.println(soma);
        } else if (indice < vetor.length){
            if(vetor[indice] % 2 != 0) {
                soma += vetor[indice];
                indice++;
            }
            somaImparRecursivo(indice, soma);
        }
        return -1;
    }
    ...
    /* Método ordena - ordena o conteúdo da lista de forma crescente (do menor para o maior)
       utilizando InsertionSort, ou MergeSort ou QuickSort.
       Se for necessário, pode criar outros métodos que são chamados pelo método ordena.
     */

    public void ordena() {
        insertionSort(vetor, nroElem);
    }

    public void insertionSort(int[] v, int nEle){
        int x, y, k;
        for (x = 1; x < nEle; x++) {
            k = v[x];
    ...
    public void listaParaMatriz(int[][] m) {
        if (m.length * m[0].length != nroElem) throw new IllegalArgumentException();
        int k = 0;
