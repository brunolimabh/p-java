...
public class ClienteResource {
	...
	public RespostaCpf lambdaValidarCpf(String cpf) throws JsonProcessingException {
		Map<String, String> parametros = Map.of("cpf", cpf);
		...
		return objectMapper.readValue(value, RespostaCpf.class);
	}
	...
	@PostMapping
	public ResponseEntity<Cliente> criar(@RequestBody @Valid Cliente cliente) throws JsonProcessingException {
		RespostaCpf rCpf = lambdaValidarCpf(cliente.getCpf());
		if (!rCpf.valido()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, rCpf.resultado());
		repository.save(cliente);
		return ResponseEntity.status(201).body(cliente);
	}

	@PutMapping("/{id}")
	public ResponseEntity<Cliente> atualizar(@PathVariable long id, @RequestBody @Valid Cliente cliente) throws JsonProcessingException {
		RespostaCpf rCpf = lambdaValidarCpf(cliente.getCpf());
		if (!rCpf.valido()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, rCpf.resultado());
		if (!repository.existsById(id)) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND);
		}
		if (!lambdaValidarCpf(cliente.getCpf()).valido()) return ResponseEntity.badRequest().build();
